...
const Ed = {
  tab: "monsters",
  pane: "list",         // which pane is showing on a narrow screen
  ...
  /** Only one pane is visible on a phone; the body attribute picks it. */
  setPane(p) {
    this.pane = p;
  ...
  init() {
    ...
    this.root.innerHTML =
      '<header class="edTop">' +
        ...
        '<button class="btn ghost sm desktopOnly" id="edSeed">Seed defaults</button>' +
        '<button class="btn ghost sm desktopOnly" id="edImport">Import</button>' +
        '<button class="btn ghost sm desktopOnly" id="edExport">Export JSON</button>' +
        '<a class="btn sm ghost desktopOnly" href="mapeditor.html">Map editor</a>' +
        '<a class="btn sm desktopOnly" id="edPlay" href="index.html">Open game →</a>' +
        '<button class="btn sm ghost mobileOnly" id="edMenu" aria-label="Menu">⋯</button>' +
      "</header>" +
      '<div class="edBody">' +
    ...
    $("#edMenu").onclick   = () => this.mobileMenu();

    this.setPane("list");
    // Coming back to a wide window should restore both panes.
    window.addEventListener("resize", () => {
      if (!this.compact() && this.pane !== "list") this.setPane("list");
    });

    this.renderAll();
  },

  /** Pull data/*.json in and seed anything still empty, then redraw. */
  async loadDb() {
    await DB.load();
    DB.seedAll(false);
    if (DB.problems.length) {
      this.toast("Some database files did not load — run `npm run serve` and open over http.", "bad");
  ...
  renderAll() {
    document.body.setAttribute("data-pane", this.pane);
    this.renderTabs();
    this.renderListBar();
    this.renderList();
    this.renderForm();
    ...
    if (this.tab === "monsters") {
      ...
      body = rows.map(m => {
        ...
        return '<tr data-id="' + m.monsterId + '" class="' + (this.selected === m.monsterId ? "on" : "") + '">' +
          '<td data-l="Name"><div class="nameCell"><span class="emojiIcon">' + esc(m.icon || "👹") + "</span>" +
            "<b>" + esc(m.name || "(unnamed)") + "</b>" +
            (m.isBoss ? ' <span class="tag" style="border-color:var(--gold);color:var(--gold)">boss</span>' : "") +
            "</div></td>" +
          '<td data-l="Type"><span class="tag">' + esc(m.type) + "</span></td>" +
          '<td data-l="Rarity">' + this.rarityChip(m.rarity) + "</td>" +
          '<td class="num" data-l="Levels">' + m.levelMin + "–" + m.levelMax + "</td>" +
          '<td class="num" data-l="HP">' + e.hp + "</td>" +
          '<td class="num" data-l="Attack">' + e.atkMin + "–" + e.atkMax + "</td>" +
          '<td class="num" data-l="XP">' + e.exp + "</td>" +
          '<td data-l="Loot table">' + (lt ? esc(lt.name) : '<span class="dimmer">—</span>') + "</td></tr>";
      }).join("");
      ...
      body = rows.map(t => {
        ...
        return '<tr data-id="' + t.lootTableId + '" class="' + (this.selected === t.lootTableId ? "on" : "") + '">' +
          '<td data-l="Name"><b>' + esc(t.name || "(unnamed)") + "</b></td>" +
          '<td class="num" data-l="Entries">' + (t.loot || []).length + "</td>" +
          '<td class="num" data-l="Total %">' + total.toFixed(1) + "</td>" +
          '<td class="num" data-l="Drops">' + t.dropsMin + "–" + t.dropsMax + "</td>" +
      ...
      body = rows.map(t => {
        ...
        let top = -1, topI = -1;
        odds.forEach((o, i) => { if (o > top) { top = o; topI = i; } });
        const topMon = topI >= 0 ? Content.get("monsters", t.monsters[topI]) : null;
        const names = (t.monsters || []).slice(0, 3).map(id => {
          const m = Content.get("monsters", id);
          return m ? esc(m.name) : "?";
        }).join(", ");
        const more = (t.monsters || []).length > 3 ? " +" + ((t.monsters || []).length - 3) : "";
        return '<tr data-id="' + t.spawnTableId + '" class="' + (this.selected === t.spawnTableId ? "on" : "") + '">' +
          '<td data-l="Name"><b>' + esc(t.name || "(unnamed)") + "</b></td>" +
          '<td class="num" data-l="Monsters">' + (t.monsters || []).length + "</td>" +
          '<td class="num" data-l="Pack">' + t.packMin + "–" + t.packMax + "</td>" +
          '<td data-l="Most likely">' + (topMon ? esc(topMon.name) + ' <span class="dimmer">' + Math.round(top * 100) + "%</span>"
      ...
      body = rows.map(it => {
        ...
        return '<tr data-id="' + it.itemId + '" class="' + (this.selected === it.itemId ? "on" : "") + '">' +
          '<td data-l="Name"><div class="nameCell"><span class="iconBox" style="color:' + Content.rarityColor(it.rarity) + '">' +
            Content.iconSvg(it.iconKey, 18) + "</span><b>" + esc(it.name || "(unnamed)") + "</b></div></td>" +
          '<td data-l="Slot"><span class="tag">' + esc(slot.label) + "</span></td>" +
          '<td data-l="Rarity">' + this.rarityChip(it.rarity) + "</td>" +
          '<td class="num" data-l="Damage">' + dmg + "</td>" +
          '<td class="num" data-l="Armour">' + (+it.armor || '<span class="dimmer">—</span>') + "</td>" +
          '<td class="num" data-l="Res">' + (+it.resistance || '<span class="dimmer">—</span>') + "</td>" +
          '<td class="num" data-l="Level">' + (it.itemLevel || 1) + "</td>" +
          '<td class="num" data-l="Value">' + (it.value || 0) + "</td></tr>";
      }).join("");
    }
  ...
  save() {
    ...
    const idKey = Content.ID_KEY[this.tab];
  ...
  validate(d) {
    ...
    if (this.tab === "spawns") {
      if (+d.packMin > +d.packMax) return "Minimum pack size is above the maximum.";
      if (+d.packMin < 1) return "A pack needs at least one monster.";
      if ((d.monsters || []).some(id => !id)) return "Every row needs a monster selected.";
      if ((d.weights || []).some(w => !(+w >= 0))) return "Weights cannot be negative.";
    }
    if (this.tab === "loot") {
  ...
  del() {
    ...
    if (this.tab === "monsters") {
      Content.list("spawns").forEach(t => {
        const keep = [], keepW = [];
        (t.monsters || []).forEach((mid, i) => { if (mid !== id) { keep.push(mid); keepW.push(t.weights[i]); } });
        if (keep.length !== (t.monsters || []).length) { t.monsters = keep; t.weights = keepW; Content.save("spawns", t); }
      });
    }
    if (this.tab === "spawns") {
      Content.list("locations").forEach(l => {
        if (l.spawnTableId === id) { l.spawnTableId = ""; Content.save("locations", l); }
      });
    }
    if (this.tab === "loot") {
      Content.list("locations").forEach(l => {
        if (l.chestLootTableId === id) { l.chestLootTableId = ""; l.chestTier = ""; Content.save("locations", l); }
      });
    }
    if (this.tab === "items") {
...
Object.assign(Ed, {
  ...
  formHeader(title) {
    return '<div class="formHead">' +
      '<button class="backBtn" id="edBack" aria-label="Back to the list">←</button>' +
      "<h3>" + esc(title) + "</h3></div>";
  },
  ...
  wireActions() {
    const back = $("#edBack");
    if (back) back.onclick = () => { if (!this.dirtyCheck()) this.setPane("list"); };
  ...
  seed() {
    ...
    if (!empty && !confirm("Replace everything with the contents of data/*.json?\n\nAnything you have authored will be lost.")) return;
    DB.seedAll(true);
    this.selected = null; this.draft = null;
    this.renderAll();
    const s = Content.stats();
    this.toast("Reseeded from data/*.json — " + s.monsters + " monsters, " + s.loot +
               " loot tables, " + s.items + " items.", "good");
  },
...
window.ED = { Ed, Content, DB, Store };

function edBoot() { Ed.init(); Ed.loadDb(); }
if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", edBoot);
else edBoot();

/* ==========================================================================
   SPAWN TABLE FORM
   Weights are relative, not percentages: 30 against 10 turns up three times as
   often. Unlike loot chances they are normalised, because picking one of a set
   is what a weight means — so the derived probability is shown on every row.
   ========================================================================== */
Object.assign(Ed, {
  spawnForm(host) {
    const d = this.draft;
    if (!d.monsters) d.monsters = [];
    if (!d.weights) d.weights = [];

    host.innerHTML =
      this.formHeader(this.isNew ? "New spawn table" : d.name) +
      this.fld("f_name", "Name", d.name, { placeholder: "Loading Dock Vermin" }) +
      '<div class="sect">Pack size</div>' +
      '<div class="row2">' +
        this.fld("f_packMin", "Minimum", d.packMin, { type: "number", min: 1 }) +
        this.fld("f_packMax", "Maximum", d.packMax, { type: "number", min: 1 }) +
      "</div>" +
      '<div class="sect">Monsters</div>' +
      '<div id="edSpawnRows"></div>' +
      '<button class="btn ghost block sm" id="edAddSpawn" style="margin-top:6px">+ Add monster</button>' +
      '<div class="lootSum" id="edSpawnSum"></div>' +
      '<div class="noteBox">Weights are relative. A monster on 30 beside one on 10 shows up ' +
        "three times as often; the percentage beside each row is what that works out to. " +
        "Each slot in the pack is drawn independently, so the same monster can turn up twice.</div>" +
      this.formActions();

    this.bind("f_name", "name");
    this.bind("f_packMin", "packMin", this.int, () => this.refreshSpawnSum());
    this.bind("f_packMax", "packMax", this.int, () => this.refreshSpawnSum());
    $("#edAddSpawn").onclick = () => {
      this.draft.monsters.push("");
      this.draft.weights.push(10);
      this.renderSpawnRows();
    };
    this.renderSpawnRows();
  },

  renderSpawnRows() {
    const host = $("#edSpawnRows");
    if (!host) return;
    const d = this.draft;
    const monsters = Content.list("monsters");
    if (!d.monsters.length) {
      host.innerHTML = '<div class="dimmer tiny" style="padding:8px 0">No monsters yet.</div>';
    } else {
      const odds = Content.spawnOdds(d);
      host.innerHTML = d.monsters.map((mid, i) => {
        const opts = '<option value="">— pick a monster —</option>' + monsters.map(m =>
          '<option value="' + m.monsterId + '"' + (m.monsterId === mid ? " selected" : "") + ">" +
          esc(m.name) + " · " + esc(Content.rarityName(m.rarity)) + " · lv " + m.levelMin + "–" + m.levelMax +
          "</option>").join("");
        return '<div class="lootRow">' +
          '<select data-spawn="' + i + '">' + opts + "</select>" +
          '<span class="pct"><input class="input" type="number" step="1" min="0" ' +
            'data-weight="' + i + '" value="' + (d.weights[i] != null ? d.weights[i] : 0) + '"></span>' +
          '<button class="xBtn" data-rms="' + i + '">✕</button>' +
          '<div class="tiny dimmer" style="grid-column:1/-1;margin:-2px 0 4px">' +
            (odds[i] * 100).toFixed(1) + "% of each pack slot</div>" +
          "</div>";
      }).join("");
    }
    $$("[data-spawn]", host).forEach(sel => {
      sel.onchange = () => { this.draft.monsters[+sel.getAttribute("data-spawn")] = sel.value; this.renderSpawnRows(); };
    });
    $$("[data-weight]", host).forEach(inp => {
      inp.oninput = () => {
        this.draft.weights[+inp.getAttribute("data-weight")] = this.num(inp.value);
        this.refreshSpawnSum();
      };
      inp.onchange = () => this.renderSpawnRows();
    });
    $$("[data-rms]", host).forEach(b => {
      b.onclick = () => {
        const i = +b.getAttribute("data-rms");
        this.draft.monsters.splice(i, 1);
        this.draft.weights.splice(i, 1);
        this.renderSpawnRows();
      };
    });
    this.refreshSpawnSum();
  ...
  refreshSpawnSum() {
    const box = $("#edSpawnSum");
    if (!box) return;
    const d = this.draft;
    const total = (d.weights || []).reduce((s, w) => s + (+w || 0), 0);
    box.innerHTML =
      "<span>" + (d.monsters || []).length + " monsters · total weight " + total + "</span>" +
      "<span>pack of " + (+d.packMin || 1) + "–" + (+d.packMax || 1) + "</span>";
  }
});
...
/* ==========================================================================
   MOBILE ACTION SHEET
   The toolbar buttons that don't fit on a phone.
   ========================================================================== */
Object.assign(Ed, {
  mobileMenu() {
    const back = el("div", "modalBack");
    const m = el("div", "modal");
    m.innerHTML = "<h3>Menu</h3><div class='mb' id='edMenuBody'></div>" +
      "<div class='mf'><button class='btn ghost' id='edMenuClose'>Close</button></div>";
    back.appendChild(m);
    document.body.appendChild(back);
    const body = $("#edMenuBody", m);
    ...
    add("Seed defaults", "ghost", () => this.seed());
    add("Import JSON", "ghost", () => this.importJson());
    add("Export JSON", "ghost", () => this.exportJson());
    const nav = el("div");
    nav.innerHTML =
      '<a class="btn block ghost" href="mapeditor.html" style="margin-bottom:8px">Map editor</a>' +
      '<a class="btn block" href="index.html">Open game →</a>';
    body.appendChild(nav);
    const s = Content.stats();
    body.appendChild(el("p", "tiny dimmer", "<br>" + s.monsters + " monsters · " + s.loot +
      " loot tables · " + s.items + " items · " + s.spawns + " spawn tables · " +
      s.locations + " locations"));
    $("#edMenuClose", m).onclick = () => back.remove();
